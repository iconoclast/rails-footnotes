module Footnotes
  module Notes
    class QueriesNote < AbstractNote
      cattr_accessor :alert_db_time, :alert_sql_number, :orm, :ignore_verbose_sql, :instance_writter => false

      @@alert_db_time         = 16.0
      @@alert_sql_number      = 8
      @@query_subscriber      = nil
      @@orm                   = [:active_record, :data_mapper]
      @@ignore_verbose_sql = false
    ...
    class QuerySubscriber < ActiveSupport::LogSubscriber
      ...
      def should_ignore_event?(event)
        return false unless QueriesNote.ignore_verbose_sql
        event.payload[:sql] =~ /FROM (pg_tables|pg_class|pg_attribute|sqlite_master)/
      end
